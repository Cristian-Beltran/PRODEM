...
export const getVerifyVehicles = async (req, res) => {
  try {
    ...
    const guardUser = await User.findOne({
      where: { type: "guard", id: verifyVehicles.guardId },
    ...
    const formattedVerifyVehicles = verifyVehicles.map((verifyVehicles) => ({
      ...
      guardName: guardUser
        ? `${guardUser.first_name} ${guardUser.last_name}`
        : "",
      driverName: verifyVehicles.Driver
        ? `${verifyVehicles.Driver.user.first_name} ${verifyVehicles.Driver.user.last_name}`
        : "",
